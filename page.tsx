  ...
  return (
    <div className="space-y-6">
      <form onSubmit={addGoal} className="card space-y-3">
        <h2 className="font-display text-lg">Nueva meta</h2>
        <div className="grid grid-cols-3 gap-3">
          <input className="input col-span-3" placeholder="Nombre (ej. Vacaciones)" value={name} onChange={(e) => setName(e.target.value)} />
          <input className="input" type="number" placeholder="Objetivo" value={target} onChange={(e) => setTarget(e.target.value)} />
          <input className="input" type="number" placeholder="Ya ahorrado" value={current} onChange={(e) => setCurrent(e.target.value)} />
        </div>
          ...
          return (
            <div key={g.id} className="card">
              ...
              <p className="text-sm text-ink/60">
                {formatMoney(g.currentAmountMinor, currency)} de {formatMoney(g.targetAmountMinor, currency)}
              </p>
            </div>
          );
        })}
        {state.goals.length === 0 && (
          <p className="text-ink/50 text-sm">Aún no tienes metas. Crea la primera arriba.</p>
        )}
      </div>
    </div>
  );
